...
<div class="container">
  ...
  @endif
  <a href="{{route('selected')}}"><button style="margin-bottom: 20px;" type="button" class="btn btn-outline-primary"><i class="fas fa-arrow-left"></i>&nbsp;Back</button></a>
  <form action="{{$action}}" method="post">
    ...
    <div class="form-group">
      ...
      <select multiple class="form-control" name="option_id" id="option_id" style="height: 300px;">
        ...
        <option onclick="changeOption(this)" class="{{$option->questions_id}}" value="{{$option->id}}" @if(!$new && $selected->options_id == $option->id) selected @endif>ID: {{$option->id}}&#160;&#160;&#160; Question ID: {{$option->questions_id}}&#160;&#160;&#160; Option: {{$option->option}}</option>
        @endforeach
      </select>
    </div>
    <button type="submit" style="margin-bottom: 40px;" class="btn btn-primary btn-sm float-right">@if($new) Add @else Save @endif Selected</button>
  </form>
</div>
